...
func TestIsValid_ValidProtocols(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := protocols.IsValid(tt.protocol)
...
func TestValidProtocols(t *testing.T) {
	result := protocols.ValidProtocols()
...
func TestProtocolStrings(t *testing.T) {
	result := protocols.ProtocolStrings()
...
func TestNewMessage_StructuredContent(t *testing.T) {
	...
	msg := protocols.NewMessage("assistant", content)
...
func TestNewMessage_Roles(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			msg := protocols.NewMessage(tt.role, "content")
			if msg.Role != tt.role {
...
func TestExtractOption_ExistsWithCorrectType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case float64:
				result := protocols.ExtractOption(options, tt.key, tt.defaultValue.(float64))
				if result != expected {
					t.Errorf("got %v, want %v", result, expected)
				}
			case int:
				result := protocols.ExtractOption(options, tt.key, tt.defaultValue.(int))
				if result != expected {
					t.Errorf("got %v, want %v", result, expected)
				}
			case string:
				result := protocols.ExtractOption(options, tt.key, tt.defaultValue.(string))
				if result != expected {
					t.Errorf("got %v, want %v", result, expected)
				}
			case bool:
				result := protocols.ExtractOption(options, tt.key, tt.defaultValue.(bool))
				if result != expected {
					t.Errorf("got %v, want %v", result, expected)
				}
			}
...
func TestExtractOption_ExistsWithWrongType(t *testing.T) {
	...
	result := protocols.ExtractOption(options, "temperature", 0.5)
	if result != 0.5 {
...
func TestExtractOption_DoesNotExist(t *testing.T) {
	...
	result := protocols.ExtractOption(options, "nonexistent", 0.5)
	if result != 0.5 {
		t.Errorf("expected default value 0.5, got %v", result)
	}
}
...
func TestExtractOption_NilOptions(t *testing.T) {
	result := protocols.ExtractOption[float64](nil, "temperature", 0.5)
	if result != 0.5 {
		t.Errorf("expected default value 0.5, got %v", result)
	}
}
